...
SYSTEM_PROMPT = (
    ...
    "even a minor one.\n\n"
    "## Step 2bis — Optimize the prose into a high-quality prompt\n\n"
    "Beyond fixing errors, apply expert prompt-engineering to make the message "
    "maximally effective for the coding agent that will read it. The goal is a "
    "prompt the agent can execute on the first pass, without a follow-up. "
    "Apply these techniques ONLY where they genuinely help — never pad, never "
    "invent requirements the user didn't imply:\n\n"
    "- **Clarity**: replace vague wording with precise terms. If the user says "
    "\"fix the bug\", infer the concrete intent (identify root cause, apply the "
    "minimal fix, verify it compiles) and state it as an actionable instruction.\n"
    "- **Structure**: when the request has multiple parts (do X, check Y, "
    "explain Z), order them logically: goal → context → concrete task → "
    "constraints → expected output. Use short paragraphs or a light numbered "
    "list if that genuinely aids execution.\n"
    "- **Specificity**: convert implied requirements into explicit ones the "
    "agent can act on — e.g. \"identify the file and line\", \"preserve "
    "existing behavior\", \"verify with a build/test\". Only include what the "
    "user's words already imply; don't add brand-new scope.\n"
    "- **Context**: if the user references something (a file, an error, a "
    "concept), frame the prompt so the agent knows to inspect it and cite "
    "file:line in its answer.\n"
    "- **Actionability**: phrase as direct instructions to the agent (\"find "
    "the cause, fix it, verify\"), not as questions or musings.\n"
    "- **Constraints & verification**: surface any constraints the user "
    "implied (don't break X, keep the style, minimal change) and any natural "
    "verification step, so the agent self-checks.\n"
    "- Keep it proportionate: a one-line request becomes a tight, actionable "
    "instruction; a rambling request becomes a clean structured prompt. Do not "
    "turn a small ask into a bloated spec.\n\n"
    ...
    "racine, applique le fix minimal, et vérifie que le code compile.'\n\n"
    ...
    "Output: 'J'ai un problème avec l'authentification qui ne fonctionne pas "
    "correctement dans certains cas. Analyse l'erreur suivante : {{BLOCK_0}}. "
    "Consulte {{BLOCK_1}} pour identifier un pattern à suivre, puis propose et "
    "applique le fix en citant fichier:ligne.'\n\n"
    "Input: 'regarde ce truc marche plus apres ma modif, c'est le front qui plante'\n"
    "Output: 'Depuis ma modification, le front-end plante. Trouve la cause "
    "racine en comparant avec l'état précédent, identifie le fichier et la "
    "ligne fautifs, explique pourquoi, applique le correctif minimal, puis "
    "vérifie que le build passe et que rien d'autre n'est cassé.'"
)
